...
function devicesCreate(Store, TID, TableNos) {
    ...
    cy.contains(TID).click();

    cy.wait(1 * 1000);
    cy.contains('디바이스 시리얼 연동').click();
    cy.wait(1 * 1000);
    // TableNos가 배열이므로 반복 처리
    TableNos.forEach(TableNo => {
        cy.contains('MK2406TO10' + TableNo).click();
        cy.wait(1 * 1000);
    });
    cy.wait(1 * 1000);
    cy.contains('선택 완료').click();
    cy.wait(1 * 1000);
    cy.contains('연동하기').click();
    cy.wait(1 * 1000);
    cy.contains('수정 하시겠습니까?').should('be.visible');
    cy.wait(1 * 1000);
    cy.contains('확인').click();
    cy.get('#vuePayContainer > .modal-dialog > .modal-content > .modal-header > .btn-close').click();
}

function devicesDelete(Store, TID, TableNos) {
    cy.get(':nth-child(3) > .container-fluid > .d-flex > [href="/account/partners"] > .btn').click(); // 계정관리
    cy.get('[href="/account/store"] > .btn').click(); // 매장관리
    cy.get('#keyword').type(Store); // 검색어
    cy.get('.card-body > .row > .my-sm-auto > .btn').click(); // 검색
    ...
    cy.get('#vuePayContainer > .modal-dialog > .modal-content > .modal-header > .btn-close').click();
    cy.wait(1 * 1000);
    cy.get(':nth-child(3) > .container-fluid > .d-flex').contains('매장관리').click();
    cy.wait(1 * 1000);
    cy.get('[href="/store/table-order/basic"] > .btn').click();
    cy.wait(1 * 1000);
    cy.get('#device').click();
    cy.wait(1 * 1000);
    cy.get('#basic-store-no').select(Store);
    cy.wait(1 * 1000);
    cy.get('.col-2 > .btn').click();
    cy.wait(1 * 1000);
    // cy.get('.monki-topmenu-a nav-link mb-0 px-0 py-1').click();
    cy.wait(1 * 1000);
    cy.get('.align-middle.text-start') // MK2406TO10 텍스트가 있는 <td> 요소 선택
        .each($cell => {
            TableNos.forEach(TableNo => {
                // 해당 셀의 텍스트 가져오기
                const rowText = $cell.text();
                if (rowText.includes('MK2406TO10' + TableNo)) {
                    // MK2406TO10 + TableNo 조합이 포함된 행 찾기
                    cy.wrap($cell) // 현재 셀 래핑
                        .closest('tr') // 해당 셀이 속한 <tr> 찾기
                        .find('button') // 그 행 내에서 '삭제' 버튼 찾기
                        .contains('삭제') // '삭제' 버튼이 포함된 요소 찾기
                        .click(); // 클릭
                    cy.wait(1000); // 1초 대기
                }
            });
        });
    cy.get('.modal.show > .modal-dialog > .modal-content').contains('디바이스를 매장에서 삭제하시겠습니까?');
    cy.get('#global_modal_confirm').click();
    cy.get('#container').contains(TableNos).should('not.be.visible');
}
module.exports = {
    store,
    menu,
